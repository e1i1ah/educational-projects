...
class StgEtlSettingsRepository:
    def get_setting_stg(self, conn: Connection, etl_key: str) -> Optional[EtlSetting]:
    ...
    def get_setting_dds(self, conn: Connection, etl_key: str) -> Optional[EtlSetting]:
        with conn.cursor(row_factory=class_row(EtlSetting)) as cur:
            cur.execute(
                """
                    SELECT
